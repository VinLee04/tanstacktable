...
export type TUserRole = 'STAFF' | 'LEADER' | 'INTERN' | 'MANAGER'

function UserRoleBadge({role, className}: { role: TUserRole, className?: string }) {

    const variant: Record<TUserRole, "primary" | "secondary" | "success" | "warning" | "info" | "outline" | "destructive" | null | undefined> = {
        'STAFF': 'success',
        'LEADER': 'info',
        'INTERN': 'destructive',
        'MANAGER': 'warning',
    ...
    return <Badge variant={variant[role]} appearance='outline'
                  className={`font-semibold w-full ${className}`}>{role}</Badge>
}

function ActiveBadge({isActive, className}: { isActive: boolean, className?: string }) {

    return <Badge variant={isActive ? 'success' : 'destructive'} appearance='outline'
                  className={`font-semibold w-full ${className}`}>{isActive ? 'AVAILABLE' : 'DISABLED'}</Badge>
}

export type TUserStatus = 'FREE' | 'BUSY';

function UserStatusBadge({status, className}: { status: TUserStatus, className?: string }) {

    const variant: Record<TUserStatus, "primary" | "secondary" | "success" | "warning" | "info" | "outline" | "destructive" | null | undefined> = {
        'FREE': 'warning',
        // 'LEADER': 'info',
        'BUSY': 'destructive',
    }

    return <Badge variant={variant[status]} appearance='outline'
                  className={`font-semibold w-full ${className}`}>{status}</Badge>
}


export {Badge, BadgeButton, BadgeDot, badgeVariants, UserRoleBadge, UserStatusBadge, ActiveBadge};
