...
import {
  ...
  DropdownMenuTrigger,
} from '@/components/table/dropdown-menu.tsx';
...
function DataGridColumnHeader<TData, TValue>({
  ...
  const headerLabel = () => {
    return (
      <div
        className={cn(
          'text-secondary-foreground/80 font-bold inline-flex h-full items-center gap-1.5 text-[0.8125rem] leading-[calc(1.125/0.8125)] [&_svg]:size-3.5 [&_svg]:opacity-60',
          className,
        )}
      >
        {icon && icon}
  ...
  const headerButton = () => {
    return (
      <Button
        variant="ghost"
        className={cn(
          'text-secondary-foreground/80 w-full uppercase justify-between rounded-none font-bold h-10 hover:bg-secondary data-[state=open]:bg-secondary hover:text-foreground data-[state=open]:text-foreground',
          className,
        )}
